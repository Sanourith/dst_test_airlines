...
def fetch_departing_flights(
    ...
    def increment_range(flight_endpoint: str) -> tuple:
        ...
        if hour_int >= 24:
            logger.info(
                f"Nouveau range dépasse 24h (heure après incrémentation : {hour_int}h)"
            )
            outranged = True
    ...
    if api_name == "flightstatus":
        ...
        logger.error(
            f"La valeur de {api_name = } n'est pas une valeur acceptée ('flightstatus' ou 'customerflightinformation')"
        )
        raise ValueError(
            f"La valeur de {api_name = } n'est pas une valeur acceptée ('flightstatus' ou 'customerflightinformation')"
        )
    ...
    flights_data_dic["metadata"]["time_at_execution"] = datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            logger.debug(
                f'Longeur du résultat : {len(flights_data[data_container]["Flights"]["Flight"])}'
            )
        ...
        ###
            ...
            logger.info(
                f"Erreur 404, pas de données sur le range de 4h commençant à : {start_time} avec pour offset : {offset}"
            )
        ...
        ###
            ...
            and response.json()["Error"] == "Account Over Queries Per Second Limit"
        ):
            time.sleep(1)
        ...
        ###
            ...
            logger.error(
                f"Erreur sur la requête de {flight_endpoint}: {response.status_code}",
                extra={"error_message": f'{response.json()["Error"]}'},
            )
            break
    ...
    for flight_resource in flight_resources:
        ...
        if new_flight_endpoint != flight_endpoint:
            if meta_total_count != table_total_count:
                ...
                logger.error(
                    f"Attention, pour le endpoint {new_flight_endpoint} (vérif : {flight_endpoint_verif}) ! La longueur du tableau ({table_total_count}) n'est pas égale au compte total des métadonnées ({meta_total_count})."
                )
...
def structure_departing_flights(file: str | dict, api_name: str) -> dict:
    ...
    flight_resources = [
        flight_resource[data_container] for flight_resource in flight_data["data"]
    ]

    consolidated_flight_data["flights"] = extract_flights_from_resources(
        flight_resources
    )
...
def download_fullday_departing_flights(
    api_token: str,
    public_ip: str,
    airport_iata: str,
    api_name: str = "flightstatus",
    ...
    flight_data = fetch_departing_flights(
        airport_iata,
        headers,
        api_name=api_name,
        date=date_formated,
    ...
    file_path = utils.build_data_storage_path(
        file_name=file_name, data_stage="interim", folder="flights"
    )
